...
exports.getMyPosts = async (req, res) => {
    try{
      ...
      let user = '';
      if(user_id){
        let total_post = await Post.find({ created_by: user_id, status:'active' });
        let posts = await Post.find({ created_by: user_id, status:'active' }).populate("video").populate("comment").populate("user").sort({_id : -1}).limit(page_size).skip(page_size * page);
        if(posts){
        ...
        const decoded = jwt.verify(token, "bezkoder-secret-key");  
        let userId = decoded.id;
        let total_post = await Post.find({ created_by: userId, status:'active' });
        let posts = await Post.find({ created_by: userId, status:'active'}).populate("video").populate("comment").populate("user").sort({_id : -1}).limit(page_size).skip(page_size * page);
        if(posts){
            await Promise.all(posts.map( async (element) => {
                if(element._id){
...
exports.postListingHome = async (req, res) => {
    try{
        ...
        var postdata = [];
        var dd = {};
        let get_post_lenth = await Post.find({ audience: 'public', status:'active' }).populate("user").populate("video");
        let post = '';
        if(token){
            post = await Post.find({ audience: 'public', status:'active' }).populate("user").populate("video").sort({_id : -1}).limit(page_size).skip(page_size * page);
            const decoded = jwt.verify(token, "bezkoder-secret-key");  
            let userId = decoded.id;
            ...
            post = await Post.find({ audience: 'public', status:'active' }).populate("user").populate("video").sort({_id : -1}).select({ likedbyme : 0 }).limit(page_size).skip(page_size * page);
        }
        if(post){
...
exports.saveBookmarkPost = async (req, res) => {
    try{
      ...
      if(checkFolder){
        ...
        if(post){
            let checkIfexists = await Folderpost.find({ post_id: post_id, folder_id: folder_id});
    ...
    try{
        ...
        if(userId){
            ...
            await Promise.all(folders.map( async (item,index)=>{
                if(item._id){
                    const f_post = await Folderpost.find({ folder_id:item._id, status:'active' });
...
exports.folderPostListing = async (req, res) => {
    try{
        ...
        if(userId && folder_id){
            let folder = await Folder.find({ _id: folder_id});
            if(folder){
...
exports.search = async (req, res) => {
    try{
        ...
        if(stringList){
            users = await User.find({username: {$regex: stringList, $options: 'i'} }).select({_id : 1, username : 1, profile_img : 1}).sort({username : 1});
        }
...
exports.createEquipment = async (req, res) => {
    try{
        ...
        let checkIFExit = await Equipment.find({name : equipment_name, created_by: userId});
        if(checkIFExit.length > 0){
